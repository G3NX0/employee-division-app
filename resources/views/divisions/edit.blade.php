...
<div class="container py-5">
    <div class="employees-hero mb-5">
        <div class="d-flex align-items-start justify-content-between gap-3 flex-wrap">
            <div>
                <h1 class="hero-title mb-2">Edit Division</h1>
                <p class="text-white-50 mb-0">Ubah nama division lalu simpan.</p>
            </div>
            <div>
    ...
    <div class="card card-modern border-0 shadow-xl">
        <div class="card-body p-4 p-lg-5">
            <form action="{{ url('divisions/' . $division->id) }}" method="POST" novalidate>
                @csrf
                @method('PUT')

                <div class="mb-3">
                    <label for="name" class="form-label">Division Name</label>
                    <input type="text" name="name" id="name" class="form-control form-control-lg @error('name') is-invalid @enderror" value="{{ old('name', $division->name) }}" placeholder="Enter division name" required>
                    @error('name')
                ...
                <div class="d-flex justify-content-end gap-2">
                    ...
                    <button type="submit" class="btn btn-primary fw-semibold px-4"><i class="bi bi-save me-2"></i>Simpan Perubahan</button>
                </div>
            </form>
        </div>
    </div>

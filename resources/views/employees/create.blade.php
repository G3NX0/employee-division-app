...
<div class="container py-5">
    <div class="employees-hero mb-5">
        <div class="d-flex flex-column flex-lg-row align-items-start align-items-lg-center justify-content-between gap-3">
            <div>
                <span class="hero-tag text-uppercase small fw-semibold">Add Employee</span>
                <h1 class="hero-title text-white fw-bold mb-2">Tambah Data Karyawan</h1>
                <p class="text-white-75 mb-0">Isi detail karyawan di bawah untuk menambah ke direktori.</p>
            </div>
            ...
            <form action="{{ route('employees.store') }}" method="POST" enctype="multipart/form-data" novalidate>
                ...
                <div class="row g-4">
                    <div class="col-md-6">
                        ...
                        <input type="text" id="name" name="name" class="form-control form-control-lg @error('name') is-invalid @enderror" placeholder="Contoh: Budi Santoso" value="{{ old('name') }}" required>
                        @error('name')
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" id="nis" name="nis" class="form-control form-control-lg @error('nis') is-invalid @enderror" placeholder="Masukkan NIS (6–12 digit)" value="{{ old('nis') }}" required>
                        @error('nis')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" id="birth_place" name="birth_place" class="form-control form-control-lg @error('birth_place') is-invalid @enderror" placeholder="Contoh: Jakarta" value="{{ old('birth_place') }}" required>
                        @error('birth_place')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-md-6">
                        ...
                        <input type="date" id="birth_date" name="birth_date" class="form-control form-control-lg @error('birth_date') is-invalid @enderror" value="{{ old('birth_date') }}" required>
                        @error('birth_date')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number" id="age" name="age" class="form-control form-control-lg @error('age') is-invalid @enderror" placeholder="Masukkan umur (tahun)" value="{{ old('age') }}" min="0" max="150" required>
                        @error('age')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="col-12">
                        ...
                        <textarea id="address" name="address" class="form-control form-control-lg @error('address') is-invalid @enderror" rows="3" placeholder="Contoh: Jl. Merdeka No. 10, Bandung, Jawa Barat">{{ old('address') }}</textarea>
                        @error('address')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="row g-4 mt-1">
                    <div class="col-md-6">
                        <label for="photo" class="form-label">Foto (opsional)
                            <i class="bi bi-info-circle ms-1" data-bs-toggle="tooltip" title="Unggah foto karyawan (JPG/PNG, maks 2MB)"></i>
                        </label>
